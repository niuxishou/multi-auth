...
@if($applicant->status != 'ワーカー登録済')
	<p style="font-size:16px;"><a href="/stylistadmin/interview_applicant?applicant_id={{ $applicant->id }}">面接結果を登録する</a></p>
@endif
@if (count($errors) > 0)
...
@endif
                <form method="POST" action="/stylistadmin/edit_applicant?applicant_id={{ $applicant->id }}" accept-charset="UTF-8">
                    {{ csrf_field() }}
