...
<div class="card">
            ...
            <div class="card-body">
               ...
               <form method="post" action="/admin/list_worker">
               	{{ csrf_field() }}
               	<input type="text" name="name" placeholder="名前" value="{{ $name }}">
               	<input type="text" name="pref" placeholder="住所（都道府県）" value="{{ $pref }}">
                ...
                <table id="account_list" class="table table-striped">
                    ...
                    @foreach ($worker_list as $worker)
                        <tr>
                            <td><a href="/admin/view_worker?worker_id={{$worker->id}}">詳細</a></td>
                            <td>{{$worker->name}}</td>
