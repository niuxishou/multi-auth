...
<html lang="ja">
    ...
    <body>
        ...
        <div class="condition">
           ...
           <li><a href="/search_district/{{ $key }}">{{ $key }}</a>({{ $value }})</li>
           @endforeach
           </ul>
        </div>
        <div class="showstylists">
        	...
        	@foreach ($worker_list as $worker)
        		<li><img src="image/see1.jpeg" alt="プロフィール画像"><br>カットの経験20年以上。皆様の理想の髪型を実現いたします！<br><a href="search_pref/{{$worker->pref}}">{{$worker->pref}}</a> &gt; {{$worker->address_1}}<br>対応可能年齢：20代〜40代</li>
        	@endforeach
        	</ul>
        </div>
        <div id="footer">
